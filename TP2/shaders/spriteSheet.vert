...
vec4 lighting(vec4 vertex, vec3 E, vec3 Norm) {
    ...
    for (int i = 0; i < NUMBER_OF_LIGHTS; i++) {
        if (uLight[i].enabled) {
            ...
            float lambertTerm = max(dot(Norm, L), 0.0);
...
void main() {
	...
	// vec3 Norm = normalize(vec3(uNMatrix * vec4(aVertexNormal, 1.0)));

    // vec3 eyeVec = -vec3(vertex.xyz);
    // vec3 E = normalize(eyeVec);

    // vFinalColor = lighting(vertex, E, Norm);
